public class ScareEverything {
    ...
    public static bool Condition(Creature? crit) {
        if (crit != null && crit.Template.type == CreatureTemplate.Type.Slugcat) {  // change the 'Slugcat' part into any creature template you want the creatures to be afraid of
    ...
    public static void Apply() {    // use this code by writing 'ScareEverything.Apply();' in the OnEnable of your mod
        ...
        On.ArtificialIntelligence.StaticRelationship += (orig, self, otherCreature) => {
            if (Condition(otherCreature.realizedCreature)) {    // if a creature skips calling DynamicRelationship in favor of using a Static one, catch it here and make sure to still return a new relationship
                return new CreatureTemplate.Relationship(newRelation, intensity);
            }
